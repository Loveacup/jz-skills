#!/usr/bin/env python3
"""Convert Obsidian Markdown to PDF using Playwright Chromium headless.

Feature-equivalent with md2pdf_chrome.py but uses Playwright for rendering,
which provides displayHeaderFooter with page numbers.
"""
...
def embed_local_images(md_text, md_dir):
    """Convert local image paths to base64 data URIs for reliable rendering."""
        ...
        if m:
            ...
            if fold == "-":
                ...
                if ct == "multi-column" and not title:
                    result.append(
                        '<div class="callout callout-multi-column" style="background:#f8f9fa;border:1px dashed #ccc;padding:8px 12px;margin:10px 0;border-radius:6px;">'
                    )
...
def build_html(md_path, header_text, directives=None, theme="blue"):
    ...
    # Mermaid JS (only if needed) — uses .replace() instead of f-string
    mermaid_script = ""
    if has_mermaid:
...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11/build/styles/{theme_obj.hljs_theme}.min.css">
...
def md_to_pdf(md_path, pdf_path=None, header_text=None, directives=None, theme="blue"):
    """Convert markdown to PDF via Playwright Chromium headless."""
    md_path = Path(md_path)
    ...
    # Escape header_text for JS string (handle quotes and backslashes)
    header_js = header_text.replace("\\", "\\\\").replace("'", "\\'")
    pdf_path_js = str(pdf_path).replace("\\", "\\\\").replace("'", "\\'")
    html_path_js = str(html_path).replace("\\", "\\\\").replace("'", "\\'")
    pdf_name_js = pdf_path.name.replace("\\", "\\\\").replace("'", "\\'")

    # Wait time: give JS time to execute (mermaid needs more)
    wait_ms = 20000 if has_mermaid else 5000

    # Playwright Node.js script for PDF generation with header/footer
    # Uses regular string concatenation for the JS, no f-string
    js_script_parts = [
        "const { chromium } = require('playwright');\n",
        "(async () => {\n",
        "  const browser = await chromium.launch();\n",
        "  const page = await browser.newPage();\n",
        "  await page.goto('file://", html_path_js, "', { waitUntil: 'networkidle' });\n",
        "  // Wait for JS pagination engine to run\n",
        "  await page.waitForTimeout(", str(wait_ms), ");\n",
        "  await page.pdf({\n",
        "    path: '", pdf_path_js, "',\n",
        "    format: 'A4',\n",
        ...
        "  await browser.close();\n",
        "  const fs = require('fs');\n",
        "  const stats = fs.statSync('", pdf_path_js, "');\n",
        "  console.log('OK ' + '", pdf_name_js, "' + ' (' + Math.round(stats.size/1024) + ' KB)');\n",
        "})();\n",
    ]
    js_script = "".join(js_script_parts)

    js_path = Path("/tmp") / "gen_pdf.js"
...
if __name__ == "__main__":
    ...
    if not positional:
        print(
            "Usage: python md2pdf_browser.py <md_file> [pdf_file] [header_text] [--theme blue|dark|academic] [--sm PATTERN] [--xs PATTERN] [--sm-after PATTERN] [--xs-after PATTERN]"
        )
        sys.exit(1)
    md_to_pdf(
        positional[0],
