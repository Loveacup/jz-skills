...
> 当前：v1.40.0 · 测试 24/24 文件、234/234 断言 · 健康分 0.96+。
...
- **TDD 红线**：实现类先写测试 → 确认失败 → 写代码 → 确认通过。当前 **234/234**。
...
- `scripts/` — 核心脚本 + cc-send-robust/cc-usage/cc-gc/cc-topic-map/cc-dogfood-report/eval + `gate/`。`tests/` — 24 文件 234/234。`references/` — 设计依据 / Hook 事实 / 事件驱动 / R2.1 澄清协议 / R8b 配置指南 / R4c smoke 流程 等。
