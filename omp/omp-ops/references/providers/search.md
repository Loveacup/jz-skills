...
| `firecrawl` | none for explicitly selected keyless search; `FIRECRAWL_API_KEY` for automatic search eligibility and the `providers.fetch` URL reader | `FIRECRAWL_BASE_URL` overrides the API endpoint (`FIRECRAWL_API_URL` is a fallback alias); the automatic search chain remains credential-gated, and fetch requires the API key | `omp search --provider firecrawl "test"` |
