...
**Progressive-disclosure search routing.** This file is ~150 lines.

> 🆕 **v3.4 (2026-05-28)**: 基于好伴AI深度研究案例 RCA，新增 3 条 deep loop Red Flag 与 4 条质量验证清单（事实解耦/Claim 溯源/补搜回路/口径确认）。详见 `references/deep-research-loop.md` 与 `references/deep-loop-verification-pattern.md`。 Detailed mode descriptions, query patterns, academic lane policy, and schema live in `references/` — loaded only when needed.
