...
| `no_agent` script timeout blamed on the model | `no_agent=true` runs the script directly; null model/provider and empty prompt means GPT was never called. Diagnose child processes, cron output artifacts, and orphan PPID=1 processes first. See `references/no-agent-script-timeout-diagnosis.md`. |
| Cron script uses `kill 0` in traps | When the scheduler runs scripts inside the gateway process group, `kill 0` can SIGTERM the gateway and trigger repeated "down" notifications. Use child-only cleanup such as `pkill -TERM -P $$ 2>/dev/null || true`; verify gateway PID and SIGTERM log count before/after a direct smoke run. |
| `no_agent=true` script prints idle output | Non-empty stdout is delivered to the user every tick. In quiet mode, redirect noisy helper stdout to `/dev/null` and verify `stdout_len=0`; stderr may be kept for diagnostics, but successful idle runs should be stdout-silent. |
| Cron script path passed as absolute path | `cronjob(action="create", script=...)` requires a path relative to `~/.hermes/scripts/` (e.g. `openwrt_watchdog.py`), not `/Users/.../.hermes/scripts/openwrt_watchdog.py` or `~/.hermes/scripts/...`. Place the script under `~/.hermes/scripts/` and pass only the filename/subpath. |
| Profile-scoped cron script placed only in default scripts dir | When `cronjob(..., profile="cron-worker", script="foo.py")` is used, the scheduler resolves `foo.py` under `~/.hermes/profiles/cron-worker/scripts/`, not the default profile's `~/.hermes/scripts/`. Copy/install the script into the target profile's `scripts/` directory and run it once from that path before declaring the job healthy. |
