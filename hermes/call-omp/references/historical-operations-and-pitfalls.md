...
> **非当前操作合同。** 本文件保存 v0.7.x 及以前从主 `SKILL.md` 迁出的完整历史说明、事故复盘与旧接口记录。
> 当前行为以 `../SKILL.md`、脚本和测试为准；遇版本、默认通道或安全规则冲突时，不得采用本文件。

# omp

> Bundle-only 后审快速操作卡（`evidence_bundle.path`、`--out`、20MB raw 熔断、无 verdict 降级）：[`references/bundle-only-audit-gates.md`](bundle-only-audit-gates.md)。完整机制仍以现有 runaway/scope references 为准。
>
> 审计 scope 的真实路径合同（语义 `include` 会过结构 gate 但产生空可读 scope）：[`references/audit-scope-real-path-contract.md`](audit-scope-real-path-contract.md)。
