...
| "本地能回答的问题别上公网 / I'll just hit the web, it's faster" ★ | **跳过 Step 0 是 v3.6 测试 P1 缺陷的根因。** Hindsight/session/qmd/CodeGraph 已沉淀过往结论、verbatim quote、user-validated facts —— 跳过 = 重新付一遍 token + 把已验证事实降级为"再次查证"。公网召回的还可能与本地结论矛盾，反而引入冲突。**强制 4 步本地检查（Hindsight → session_search → qmd/Obsidian → CodeGraph），全部 miss 才上公网。** 不查本地 ≠ 答得快；只是把 token 账单和幻觉风险一起放大。 |
...
- **Step 0.1 — Hindsight (cross-session memory)**
  - check: 用户/项目过往 session 是否已问过同一议题，是否已有结论 / source map / facts.jsonl 可复用
  - tool: `mcp_hindsight_*` search / `memory_search` / `claude-mem` search
  - skip only if: 议题明显是实时性新闻（今日股价、刚发生的事件）且 < 24h
  - escalate to public if: 命中条目 < 2 条 OR 命中结论已过期（> 90 天且涉及版本 / 价格 / 排名）
...
5. **Skipping local truth.** Check Hindsight/qmd/CodeGraph before public web.
...
- [ ] **Step 0 四步全跑过？** Hindsight / session_search / qmd-Obsidian / CodeGraph 四项是否在回答中显式声明"已查 + 命中 / 未命中 / 跳过原因"？— 未声明视同未跑（v3.6 P1 Local-first 缺陷）。
