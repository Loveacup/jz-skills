...
The Codex lane supports two model tiers, controlled by the Kanban card's `metadata.model` field:

- **Cheap (default):** `gpt-5.3-codex-spark` — for mechanical chores: frontmatter fixes, tag normalization, template filling, bounded single-file edits. Spark has generous quota and is the default for any task without an explicit model override.
- **Strong:** `gpt-5.5` — for complex multi-file changes, security-sensitive work, or tasks requiring deeper reasoning.

The worker reads `metadata.model` from the Kanban card and passes it to `codex exec --model <id>`. Cards without the field default to `cheap`.
...
```python
terminal(
    command="codex exec --sandbox workspace-write '$(cat /tmp/codex_prompt.md)'",
    workdir=WORKTREE,
    background=True,
    pty=True,
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `codex exec --sandbox workspace-write` inside the isolated worktree, then rely on Hermes reconciliation.

## Prompt Construction

Use `templates/phase0-codex-lane-card.md` when writing an implementation card plus child review card for Phase 0 / low-risk Codex-lane validation. It encodes the parent-done review-gate rule and the strict `metadata.codex_lane.result` enum.
...
### Phase 0 failure-control loop

Codex is a kill-only lane in Phase 0. Do not design around mid-task steering until a future lane proves it can reliably accept corrective input. The minimum control loop is:

1. **Monitor:** poll/log the process and inspect the worktree diff/status.
2. **Detect:** timeout, stuck/no useful output, off-scope file changes, secret/permission requests, or unrelated rewrites.
3. **Kill:** terminate the Codex process with `process(action="kill", session_id=session_id)` when a kill condition fires.
4. **Record evidence:** write a Kanban comment / metadata note with the concrete trigger, elapsed time, last useful log lines, and changed-file summary.
5. **Recover:** retry with a narrower prompt if the failure is recoverable; otherwise `kanban_block` with the rejected reason and artifact paths.

Do not silently retry. Every kill or timeout must leave enough evidence for Regent/Taizi review and for the next run to avoid repeating the same failure. For concrete `metadata.codex_lane.failure_control` examples, read `references/failure-control-metadata-example.md`.
