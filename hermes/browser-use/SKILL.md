...
On Alex's Mac, the main Chrome flow works after manual authorization in `chrome://inspect/#remote-debugging`:
...
If the main Chrome handshake fails or a clean browser profile is needed, use the verified isolated fallback:
...
The fallback launches an isolated Chrome profile under `~/.config/browser-harness/isolated-chrome-profile` on port `9223` and sets `BU_CDP_URL` automatically. Stop/reload it before claiming to operate the user's main logged-in Chrome.
