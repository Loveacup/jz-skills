...
def test_sparse_comments_do_not_claim_population_level_differences():
    body = write_comments_section({
        "evidence": [{
    ...
    assert "样本不足" in body
    assert "通常比弹幕" not in body
    assert "评论倾向于" not in body
