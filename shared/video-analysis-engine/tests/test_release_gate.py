...
def test_build_commands_default_is_fixture_gate_then_pytest():
    ...
    assert "--publishable" in fixture
    assert "--section-qa-gate" in fixture
    assert fixture[fixture.index("--depth-profile") + 1] == "claim-first-full"
    assert "--claim-qa-gate" in fixture
...
def test_build_commands_real_sample_is_opt_in_and_model_backed():
    ...
    assert "--publishable" in real
    assert "--section-qa-gate" in real
    assert real[real.index("--depth-profile") + 1] == "claim-first-full"
    assert "--claim-qa-gate" in real
