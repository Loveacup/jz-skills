...
def test_default_config_is_auto_zh(tmp_path):
    cfg = fsa.resolve_asr_config(
        env={"VIDEO_ANALYSIS_ASR_ENDPOINT_FILE": str(tmp_path / "missing-endpoint")}
    )
...
def test_endpoint_file_is_used_when_env_endpoint_missing(tmp_path):
    endpoint_file = tmp_path / "asr_endpoint"
    endpoint_file.write_text("http://asr.example.test/transcribe\n")
    cfg = fsa.resolve_asr_config(env={"VIDEO_ANALYSIS_ASR_ENDPOINT_FILE": str(endpoint_file)})
    assert cfg.endpoint == "http://asr.example.test/transcribe"


def test_invalid_provider_rejected():
    with pytest.raises(ValueError):
        fsa.resolve_asr_config(env={"VIDEO_ANALYSIS_ASR_PROVIDER": "bogus"})


def test_invalid_language_rejected():
    with pytest.raises(ValueError):
        fsa.resolve_asr_config(env={"VIDEO_ANALYSIS_ASR_LANGUAGE": "fr"})


def test_config_reads_all_env_vars():
    cfg = fsa.resolve_asr_config(env={
        "VIDEO_ANALYSIS_ASR_PROVIDER": "mlx_whisper",
        "VIDEO_ANALYSIS_ASR_MODEL": "mlx-community/whisper-tiny",
        "VIDEO_ANALYSIS_ASR_MODEL_PATH": "/Users/foo/models/snap",
        "VIDEO_ANALYSIS_ASR_LANGUAGE": "en",
        "VIDEO_ANALYSIS_ASR_ENDPOINT": "http://asr.example/transcribe",
    })
...
def test_canonical_asr_env_wins_over_legacy():
    cfg = fsa.resolve_asr_config(env={
        "VIDEO_ANALYSIS_ASR_PROVIDER": "h200_asr",
        "VIDEO_ANALYSIS_ASR_LANGUAGE": "en",
        "VIDEO_ANALYSIS_ASR_ENDPOINT": "http://canonical/transcribe",
        "BILI_ASR_PROVIDER": "mlx_whisper",
        "BILI_ASR_LANGUAGE": "zh",
        "BILI_ASR_ENDPOINT": "http://legacy/transcribe",
    })
    assert cfg.provider == "h200_asr"
    assert cfg.language == "en"
    assert cfg.endpoint == "http://canonical/transcribe"


def test_legacy_asr_env_warns():
    with pytest.warns(DeprecationWarning, match="BILI_ASR_PROVIDER"):
        cfg = fsa.resolve_asr_config(env={"BILI_ASR_PROVIDER": "whisper_cpp"})
    assert cfg.provider == "whisper_cpp"
...
def test_public_label_hides_absolute_model_path():
    cfg = fsa.AsrConfig(
        provider="whisper_cpp",
        model=None,
        model_path="/Users/example/Library/.../ggml-large-v3-turbo.bin",
        language="zh",
    )
