...
def test_logic_chain_uniformly_samples_long_timeline_to_ten_rows():
    evidence = [
        _logic_candidate(f"第 {i} 段证据。", i * 15)
        for i in range(25)
    ]
    body = write_logic_chain_section({"evidence": evidence})
    table_rows = [line for line in body.splitlines() if line.startswith("|")][2:]
    assert len(table_rows) == 10
    assert "?t=0" in table_rows[0]
    assert "?t=360" in table_rows[-1]
