...
def test_manifest_is_ten_real_samples_with_one_accepted_gold():
    ...
    gold = [s for s in samples if s["rubric"]["status"] == "accepted_gold"]
    candidates = [s for s in samples if s["rubric"]["status"] == "candidate"]
    assert len(gold) == 1
    ...
    pi = gold[0]
    assert pi["bvid"] == "BV14fTc6TEi5"
    assert pi["input"]["cache_status"] == "present"
    assert len(pi["input"]["sha256"]) == 64
    assert pi["input"]["fetch_all_json_path"].startswith("skill://")
    assert run_quality_gate._resolve_manifest_locator(
        pi["input"]["fetch_all_json_path"], str(MANIFEST)
    ).is_file()
    assert run_quality_gate._resolve_manifest_locator(
        pi["run"]["summary_json_path"], str(MANIFEST)
    ).is_file()
    assert pi["run"]["publishable_passed"] is True
    assert pi["rubric"]["reviewed_by"]
    assert pi["rubric"]["reviewed_at"]
    assert pi["rubric"]["verdict_source"]
...
def test_dry_run_does_not_call_report_markdown(monkeypatch):
    ...
    assert summary["status_distribution"] == {"accepted_gold": 1, "candidate": 9}
...
def test_cli_dry_run_needs_no_input_and_emits_json():
    ...
    assert payload["status_distribution"] == {"accepted_gold": 1, "candidate": 9}
