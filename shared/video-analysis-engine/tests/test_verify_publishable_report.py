...
def test_publishable_gate_rejects_evidence_placeholder_residue():
    md = _good_report().replace(
        "- 它保留了对证据边界的提醒。",
        "- 当前只有 [I13] E_placeholder，等待真实受众证据。",
    )
    results, passed = verify_publishable_report.evaluate(md)
    assert passed is False
    assert results["P0_NO_PLACEHOLDER_RESIDUE"]["pass"] is False


def test_publishable_gate_rejects_overlong_analysis_sections():
    oversized = ("这是用于验证最大篇幅的完整分析句子。" * 20 + "\n") * 20
    md = _good_report().replace("## 4. 内容深度拆解", oversized + "\n## 4. 内容深度拆解")
    results, passed = verify_publishable_report.evaluate(md)
    assert passed is False
    assert results["P1_SECTION_LENGTH_BOUNDS"]["pass"] is False
    assert any(item["section"] == "3" for item in results["P1_SECTION_LENGTH_BOUNDS"]["measured"])
...
def test_report_gate_accepts_single_disclaimer_in_audience_section():
    markdown = (
        "## 2. 弹幕深度分析\n"
        "_数据不足：未提供弹幕，无法判断共识度或典型反应。_\n\n"
        "## 3. 核心洞察\n正文\n\n"
        "## 7. 批判与行动\n正文"
    )
    report = {
...
def test_report_gate_does_not_join_disclaimer_fragments_across_sections():
    markdown = (
        "## 2. 弹幕深度分析\n_未提供弹幕。_\n\n"
        "## 2.5 评论分析\n评论样本不足，无法推断评论群体。\n\n"
        "## 3. 核心洞察\n正文"
    )
    report = {"frontmatter": {"danmaku_count": 0}}
    result = verify_publishable_report.evaluate_sparse_social_evidence(markdown, report)
    assert result["passed"] is False
