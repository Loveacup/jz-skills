...
def write_logic_chain_section(section_context: Dict[str, Any]) -> str:
    ...
    if len(candidates) > 10:
        ...
        sample_indices = [round(index * last / 9) for index in range(10)]
        candidates = [candidates[index] for index in sample_indices]
...
def write_comments_section(section_context: Dict[str, Any]) -> str:
    ...
    # 与弹幕差异：样本不足时只报告边界，不做群体泛化。
    lines.append('### 与弹幕差异')
    lines.append('')
    if len(comments_data) < 3:
        lines.append(
            f'- _样本不足：仅采集到 {len(comments_data)} 条评论，无法推断评论群体的长度、结构或观点倾向。_'
        )
    else:
        avg_comment_len = sum(len(c['text']) for c in comments_data) / len(comments_data)
...
_KG_LOWERCASE_TECH_TERMS = {
    'copy', 'copy all', 'diff', 'reload', 'resume', 'tx', 'update', 'yolo',
}
_KG_ENGLISH_ACRONYM_STOPWORDS = {'MD', 'PI'}
_KG_ENGLISH_ENTITY_TERMS = {
    'Pi', 'Codex', 'OpenCore', 'Marie', 'Twitter', 'ComfyUI', 'Mac', 'Windows',
    'Usage', 'Firecrawl', 'Obsidian', 'Hermes', 'Claude', 'Mermaid', 'NeoVim', 'CMax',
}
...
def _load_obsidian_moc() -> List[str]:
    """从显式配置的 Obsidian MOC 加载概念列表。

    `VIDEO_ANALYSIS_MOC_PATH` 未配置、文件不存在或为空时，使用内置高频术语；
    不猜测用户 vault 名称或目录结构。
    """
    raw_path = (os.environ.get('VIDEO_ANALYSIS_MOC_PATH') or '').strip()
    concepts = []
    if not raw_path:
        return list(_OBSIDIAN_MOC_FALLBACK)
    moc_path = os.path.expanduser(raw_path)
...
def _extract_kg_candidates(section_context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for cand in section_context.get('evidence') or []:
        ...
        if cand.get('reason') not in ('knowledge_candidate', 'application_candidate'):
            continue
        text = _clean_inline_text(cand.get('text') or '', 600)
        if not text:
            continue
        item = dict(cand)
        item['text'] = text
...
def _concepts_in_text(text: str) -> List[str]:
    ...
    for concept in _KG_CONCEPT_PATTERNS:
        if concept in text and concept not in seen:
            seen.append(concept)
    # 先显式匹配已知小写命令；避免英文 regex 把整段 ASR 碎片吞成一个“概念”。
    lowered = text.lower()
    for term in sorted(_KG_LOWERCASE_TECH_TERMS, key=len, reverse=True):
        if re.search(rf'(?<![a-z0-9]){re.escape(term)}(?![a-z0-9])', lowered) and term not in seen:
            seen.append(term)

    # 其他自动实体只接纳白名单技术名、短 acronym 或明确含内部大写的技术名。
    for entity in _extract_entities_from_text(text):
        ...
        for token in tokens:
            if len(token) == 1 or token.isdigit():
                continue
            if token.lower() in _KG_LOWERCASE_TECH_TERMS or token in _KG_ENGLISH_ACRONYM_STOPWORDS:
                continue
            if any(term.startswith(token) and term != token for term in _KG_ENGLISH_ENTITY_TERMS):
                continue
            is_readable_tech = (
                token in _KG_ENGLISH_ENTITY_TERMS
                or bool(re.fullmatch(r'[A-Z0-9]{2,6}', token))
                or bool(re.fullmatch(r'[A-Z][a-z0-9]+(?:[A-Z][A-Za-z0-9]*)+', token))
            )
            if not is_readable_tech:
                continue
            if token not in seen and len(seen) < 12:
                seen.append(token)
    return seen


def _extract_action_sentence(text: str) -> str:
    """Return one explicit, transcript-grounded action sentence or an empty string."""
    action_cues = (
        re.compile(r'(?:建议|应该).{0,60}(?:执行|创建|制作|添加|使用|检查|验证|自动化|扩展)'),
        re.compile(r'重复.{0,30}(?:三四次|[一二三四五六七八九十\d]+次).{0,60}(?:制作|创建|封装|自动化|扩展)'),
        re.compile(r'到了.{0,30}(?:制作|创建|封装).{0,20}(?:时候|时机)'),
        re.compile(r'只需.{0,50}(?:输入|执行|创建|添加|制作|运行)'),
    )
    for sentence in re.split(r'(?<=[。！？!?])\s*', text or ''):
        cleaned = sentence.strip()
        if cleaned and any(pattern.search(cleaned) for pattern in action_cues):
            return _clean_inline_text(cleaned, 180)
    return ''
...
def write_knowledge_graph_section(section_context: Dict[str, Any], max_items: int = 8) -> str:
    ...
    concepts: List[str] = []
    relations: List[str] = []
    relation_keys: List[frozenset[str]] = []
    ...
    for cand in candidates:
        ...
        if len(found) >= 2:
            ...
            linked_concepts = found[:3]
            linked = [_concept_link(c) for c in linked_concepts]
            rel = ' → '.join(linked)
            relation_key = frozenset(linked_concepts)
            if rel and not any(relation_key <= existing for existing in relation_keys):
                superseded = [
                    index for index, existing in enumerate(relation_keys)
                    if existing < relation_key
                ]
                for index in reversed(superseded):
                    relation_keys.pop(index)
                    relations.pop(index)
        ...
        if cand.get('reason') == 'application_candidate':
            applications.append(action_sentence or text)
        elif action_sentence:
            applications.append(action_sentence)
    ...
    anchor = next((str(c.get('timestamp') or '').strip() for c in candidates if c.get('timestamp')), '')
    evidence_frame = (
        f'本节基于转录时间锚点 {anchor} 附近的知识与应用证据，整理概念而不扩写视频之外的事实。'
        if anchor
        else '本节基于当前转录中的知识与应用证据，整理概念而不扩写视频之外的事实。'
    )
    reasoning_frame = (
        '因为概念只有在同一证据片段中共同出现时才建立关系，所以这些连线表示可复核的共现结构；'
        '其适用边界是辅助检索与后续研究，不等同于已经证明的因果关系。'
    )
    lines = [evidence_frame, reasoning_frame, '', '### 核心概念', '']
    if concepts:
    ...
    for sec in writer_ctx.get('sections', []):
        ...
        if section_id == "3" and int(((report.get("frontmatter") or {}).get("danmaku_count") or 0)) == 0:
            sparse_social_contract = (
                ...
                "danmaku_count=0。没有任何可用弹幕证据。§3 无弹幕时省略 `**受众反馈**：` 字段；"
                "不得写观众惊叹、认同、反应类型、共识度高/中/低或典型弹幕。"
                "数据不足声明由 §2 或 §7 统一承载，不要在每个洞察中重复。"
            )
...
- 每个模块正文 700–1800 个中文字符，不要输出短模块，也不要重复 §3 的定义性内容
...
- 只输出 Markdown 格式内容，不添加额外说明
- 不要重复输出本节 `## 3.` 大标题；直接从 `###` 小标题开始
- **必须输出恰好 3 个洞察小节，每个小节标题格式严格为 `### 💡 洞察 N：标题`（N 从 1 开始递增）**
- 每个洞察小节必须包含以下核心结构：
  * **定义**：一句话精准定义（≥20 字）
  * **深度解析**：原理层（为什么成立）+ 案例层（视频具体例子，含 [E#]）+ 关联层（与已有概念关系），**必须至少 150 个中文词/字符**
  * **受众反馈**：仅在有与该洞察直接相关的弹幕/评论 [E#] 时输出；无弹幕时省略该字段，不得猜测观众态度
  * **推理许可**：从证据到主张的推理规则（≥40 字）
  * **边界条件**：该主张的局限性或反证（≥30 字）
- **每个洞察正文 350–700 个中文字符**（不包括标题和 [E#] 标记本身）；禁止用重复句扩写
...
1. 输出恰好 3 个洞察，每个洞察标题必须是 `### 💡 洞察 N：标题`（N 从 1 开始）
2. 每个洞察必须包含：定义（≥20 字）、深度解析（≥150 字）、推理许可（≥40 字）、边界条件（≥30 字）；受众反馈仅在有直接相关的弹幕/评论 [E#] 时输出，无弹幕时省略该字段
3. **每个洞察正文 350–700 个中文字符**，全节保持压缩，不重复同一证据
...
- 只输出 Markdown 格式内容，不添加额外说明
- 不要重复输出本节 `## 4.` 大标题；直接从 `###` 小标题开始
- **必须输出恰好 3 个模块，每个模块标题格式严格为 `### 模块 N：标题`（N 从 1 开始递增）**
- 每个模块的递进逻辑（建议）：
  * 模块 1：现象拆解 / 定义核心概念
  * 模块 2：机制分析 / 因果链条
  * 模块 3：结构性原因 / 模式总结
- 每个模块必须包含：
  ...
  * 三层叙事标签：`**显性叙事**`（视频直接表达）、`**隐性机制**`（证据支持的机制推理）、`**元叙事**`（作者/平台如何建构议题）；三项均不可省略
  * 与用户栈关联（具体可借鉴之处）
  * 批判审视（漏洞/盲区/边界条件）
...
请根据以上证据撰写该节内容，严格遵守以下要求：
1. 输出恰好 3 个模块，每个模块标题必须是 `### 模块 N：标题`（N 从 1 开始）
2. 每个模块必须包含：核心论点、Mermaid 图、论证展开（前提→推理→结论）、`**显性叙事**`、`**隐性机制**`、`**元叙事**`、与用户栈关联、批判审视
3. 每个模块的论证展开必须至少引用 1 个 [E#] 证据编号
4. 每个模块结尾必须有一行 `证据：@[E1] @[E2]` 格式的证据引用汇总
5. 每个模块正文 700–1800 个中文字符，全节保持压缩
6. 不要输出 `## 4.` 大标题，直接从 `### 模块 1：` 开始"""
    },
    "7": {
        "system": """你是一位专业的视频分析师，负责输出批判性评估与可执行行动。
...
- 不要重复输出本节 `## 7.` 大标题；直接从 `###` 小标题开始
- 必须包含 4 个小节（即使数据稀疏也要输出框架）：
  * `### 独特价值`：恰好 3 个独特价值点，每个含 [E#] 引用；没有弹幕时不得逐条重复同一句免责声明
  * `### 局限与偏见`：恰好 2 个局限/偏见，每个含描述 + 来源分析
  * `### 弹幕共识度分析`：**必须输出此小节**；有真实弹幕才可给占比。弹幕为 0 时只写一次“弹幕数据不足，无法进行共识度分析”，不得输出伪造百分比或 placeholder
  * `### 可行动项`：恰好 3 个可行动项（立即执行 + 短期跟进 + 长期探索各 1 个），每个含证据引用 [E#] 或 [C#] 或时间戳
- `独特价值` 和 `局限与偏见` 小节下必须使用 `- ` bullet
- `可行动项` 小节下可使用 `- ` 或 `1. ` 列表或 `- [ ]` 任务列表
- 每个列表项必须包含 [E#] 或 [C#] 或时间戳引用
- 全节不超过 3500 个中文字符；优先删除重复解释，不得通过缩写证据边界来凑篇幅
- 禁止输出 placeholder、E_placeholder、I_placeholder 或任何待填证据标记
- 弹幕数据不足声明只写一次；YouTube 评论等跨平台评论可作为评论证据，但不得冒充弹幕
- 禁止编造或猜测评论区中未提及的内容
- 对不确定的信息，使用"从现有证据只能看出..."表述
- 禁止使用"显然""必然""毫无疑问"等绝对化表达""",
        "user": """# 任务：{heading}
...
请根据以上证据撰写该节内容，必须包含：
1. **独特价值**（恰好 3 个，每个含证据引用；无弹幕时不要逐条重复免责声明）
2. **局限与偏见**（恰好 2 个，每个含描述 + 来源分析）
3. **弹幕共识度分析**（有真实弹幕才输出占比；无弹幕只写一次数据不足，不得输出 placeholder）
4. **可行动项**（恰好 3 个：立即/短期/长期各 1 个，每个含证据引用 [E#] 或 [C#] 或时间戳）
5. 全节不超过 3500 个中文字符；禁止输出 placeholder、E_placeholder；弹幕数据不足声明只写一次
