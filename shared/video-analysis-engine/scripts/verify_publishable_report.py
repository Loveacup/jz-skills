...
_PLACEHOLDER_RE = re.compile(r"\b(?:E_placeholder|I_placeholder|TODO_EVIDENCE|PLACEHOLDER_EVIDENCE)\b", re.I)
_SECTION_MAX_CHARS = {"3": 5000, "4": 9000, "7": 4000}
...
def evaluate(md: str) -> Tuple[Dict[str, dict], bool]:
    ...
    placeholder_hits = sorted(set(_PLACEHOLDER_RE.findall(md)))
    results["P0_NO_PLACEHOLDER_RESIDUE"] = _result(
        not placeholder_hits,
        placeholder_hits,
        "evidence placeholder tokens are not publishable" if placeholder_hits else "ok",
    )
    ...
    overlong_sections = [
        {"section": sid, "chars": len(sections.get(sid, "")), "max_chars": limit}
        for sid, limit in _SECTION_MAX_CHARS.items()
        if len(sections.get(sid, "")) > limit
    ]
    results["P1_SECTION_LENGTH_BOUNDS"] = _result(
        not overlong_sections,
        overlong_sections,
        "analysis sections must remain compressed and reader-oriented" if overlong_sections else "ok",
    )
...
def evaluate_sparse_social_evidence(markdown: str, report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    disclaimer_re = re.compile(
        r"(?:弹幕数据不足|未提供弹幕|无可用弹幕)[^。\n]*(?:无法|不能|不足以)[^。\n]*[。.]?"
    )
    feedback = ""
    for sid in ("2", "2.5", "3", "7"):
        for paragraph in re.split(r"\n\s*\n", sections.get(sid, "")):
            match = disclaimer_re.search(paragraph)
            if match:
                feedback = match.group(0).strip()
                break
        if feedback:
            break
...
def evaluate_publishable_report(
    ...
    if not social_result["skipped"]:
        results["P0_SPARSE_SOCIAL_EVIDENCE"] = _result(
            ...
            social_result,
            "zero danmaku requires one explicit data-insufficiency disclaimer in an audience-analysis section",
        )
