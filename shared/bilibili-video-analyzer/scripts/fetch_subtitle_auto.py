...
#   BILI_ASR_PROVIDER=auto|h200_asr|whisper_cpp|mlx_whisper
#   BILI_ASR_ENDPOINT=<ASR HTTP endpoint>    （默认 SURGExZR H200）
#   BILI_ASR_MODEL=<模型名/repo id>          （主要给 mlx-whisper）
#   BILI_ASR_MODEL_PATH=<本地绝对路径/snapshot 目录>
#   BILI_ASR_LANGUAGE=zh|en|auto
ASR_PROVIDERS = ('auto', 'h200_asr', 'whisper_cpp', 'mlx_whisper')
ASR_LANGUAGES = ('zh', 'en', 'auto')
DEFAULT_H200_ASR_ENDPOINT = 'http://172.16.27.10:8088/ASR/transcribe'
...
def resolve_asr_config(env: Optional[Mapping[str, str]] = None) -> AsrConfig:
    """从环境变量解析 ASR 配置。非法 provider/language 抛 ValueError。"""
    if env is None:
        env = os.environ
    provider = (env.get('BILI_ASR_PROVIDER') or 'auto').strip().lower()
    if provider not in ASR_PROVIDERS:
        raise ValueError(
            f"BILI_ASR_PROVIDER 非法: {provider!r}，允许 {ASR_PROVIDERS}"
        )
    language = (env.get('BILI_ASR_LANGUAGE') or 'zh').strip().lower()
    if language not in ASR_LANGUAGES:
        raise ValueError(
            f"BILI_ASR_LANGUAGE 非法: {language!r}，允许 {ASR_LANGUAGES}"
        )
    model = (env.get('BILI_ASR_MODEL') or '').strip() or None
    model_path = (env.get('BILI_ASR_MODEL_PATH') or '').strip() or None
    endpoint = (env.get('BILI_ASR_ENDPOINT') or DEFAULT_H200_ASR_ENDPOINT).strip()
    return AsrConfig(
        provider=provider,
        model=model,
...
def transcribe_pages_merge(bvid, title, pages, tmpdir, output_txt_path=None):
    ...
    if output_txt_path is None:
        output_txt_path = f"/tmp/{bvid}_subtitle_whisper.txt"
