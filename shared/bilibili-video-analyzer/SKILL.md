...
- 🆕 **P2-B EvidenceMap 下一步**：Codex 只读审核确认 P2-A 方向正确但未闭环；P0 缺口是 `report_plan` 与 `render_markdown()` 脱节。下一刀必须先做 `EvidenceMap per SectionSpec` + timestamped quote/citation candidates，再考虑 LLM writer/RAG。见 `references/content-engine-p2-codex-audit-and-evidencemap-next.md`。
