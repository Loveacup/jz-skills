...
- 🆕 **DraftReport §3/§4/§7 LLM Writer Slice 记录**：`assemble_draft_report_slice(..., provider=...)` 现可显式接入既有 §3/§4/§7 LLM writer；provider 默认 `None` 不联网，合格输出写入 `draft_sections`，坏输出降级占位+warning，仍不产生 `PublishedMarkdown`。见 `references/content-engine-draft-report-slice-3-4-7-20260703.md`。
- 🆕 **DraftReport Preview Renderer 记录**：新增 `render_draft_markdown(draft)`，把 `draft_sections` 覆盖到 report plan，未填节保留 skeleton/Source Appendix，输出显式 `publishable: false` preview；不影响 `render_debug_markdown()`/`render_markdown()`。见 `references/content-engine-draft-preview-renderer-20260703.md`。
- 🆕 **DraftReport §6 Knowledge Graph Slice 记录**：新增 `write_knowledge_graph_section()`，从 transcript `knowledge_candidate`/`application_candidate` 保守抽取核心概念、关系链、可落库行动项，使用 Obsidian wikilinks；`assemble_draft_report_slice(section_ids=("6",))` 写入 `draft_sections['6']`。见 `references/content-engine-draft-report-slice-6-kg-20260703.md`。
- 🆕 **⚡ 路线切换：架构安全优先 → 内容质量优先（2026-07-03）**：暂停填补确定性小 extractor；下一步做节级内容质量闸 `evaluate_draft_section_quality()`，统一评估 evidence-grounded / not-mechanical / human-readable / insight-density / no-skeleton；之后所有 writer 都过同一个 QA gate。见 `references/content-engine-strategy-pivot-to-quality-20260703.md`。
- 🆕 **Section QA Gate Phase 1 记录**：CC 规划 + 小黄实现 + OMP 审计。新增 `SectionQualityResult`/`DimensionResult` dataclass 和 `evaluate_draft_section_quality()`（五维 D1-D5，P0 阻断/P1 警告/P2 优化三级分类）；9 项 tests 全部通过，release gate 146 passed；Phase 1 未接入 pipeline。见 `references/content-engine-section-qa-gate-phase1-20260703.md`。
- 🆕 **Section QA Gate Phase 2 记录**：Codex 规划 + CC 执行 + 小黄验收/接管小修 + OMP 审计。`DraftReport` 新增 `qa_results`；`assemble_draft_report_slice()` 对 deterministic/LLM/fallback placeholder section body 统一跑 QA，P0 skeleton blocker 不进入 `draft_sections`，P1/P2 issue 保留并 warning；targeted 21 passed，release gate 149 passed，OMP R3 pass。见 `references/content-engine-section-qa-gate-phase2-20260704.md`。
- 🆕 **Section QA Gate Phase 3 记录**：Codex 规划 + CC 执行 + 小黄验收/缓存修正 + OMP 审计。`generate_report.report_markdown()` 现在填充 JSON-able `report["section_qa"]`，`run_quality_gate.py` summary/JSON 暴露该元数据；Markdown 与 pass/fail 逻辑保持不变；`_CachingWriterProvider` 防止 QA+debug 渲染双调 LLM。targeted 12 passed，release gate 152 passed，OMP accepted with concern（OMP 无 shell runtime）。见 `references/content-engine-section-qa-gate-phase3-20260704.md`。
