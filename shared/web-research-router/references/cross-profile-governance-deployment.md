...
3. **Content parity:** each profile's copy contains key markers — `### \`academic\`` (v2 Academic Lane), `Supermemory` (v2.1 local memory), and `## Academic Lane Policy` (academic policy chapter). A profile missing these is running an outdated version.
...
stale = []
compile_fail = []
content_markers = ['### `academic`', 'Supermemory', '## Academic Lane Policy']
