...
1. Run the RPC CLI's current `devices` query and synchronize its output. This works while locked. Its `discovery_scope: "discovered_devices_and_contacts"` covers the full current discovered-user collection: own physical devices plus non-self external receiver rows identified by technical `is_contact: true`. That relation is not a human entry kind. It is not authorization or an exhaustive address book.
...
4. Present every new/unregistered live name to the user. Ask both who it belongs to—user, family/shared, another person, or unknown—and whether it is a device, contact, or still unknown. Also ask about any live legacy, unknown, or genuine stored/observed type conflict listed in `unclassified_entries` or `classification_conflicts`. Group several names into one clear question set, but do not silently defer them because a known target is usable.
5. Surface every known human-confirmed contact absent from the current snapshot through `unverified_contacts`. Absence does not delete the private record, its aliases, or its notes. It means that remembered contact is not covered by the current discovered/visible collection.
6. Await the user's answer before assigning ownership or stored entry type. Never infer either field from a display name, owner choice, alias, account resemblance, producer scope, technical `is_contact`, observed `live_entry_type`, nested child devices, or prior transfer. A new identity remains unavailable until ownership is confirmed. Entry type itself is not send permission, so a separately authorized transfer to an already confirmed, currently verified recipient may continue while unrelated coverage remains pending; disclose the pending fields.
7. Update only the exact confirmed top-level record, then synchronize relevant current output again. An external row's nested `recipient_devices` are informational reachability or exact-child choices and must never be enrolled as separate top-level inventory records. They need not have names for explicit account routing.

RPC and GUI snapshots are not expected to be identical. They expose different surfaces, and the GUI recipient surface may omit the local/self device that appears in RPC state. Reconcile exact names without treating one producer's omission as a rename or deletion. A contact-capable scope describes the full current collection that producer observed, not every contact in an account; nameless top-level receivers fail closed or remain pending, while nameless nested children stay informational.
...
Ownership and stored entry type are orthogonal. A user-owned entry is not automatically a device, an other-person entry is not automatically a contact, and an alias or technical `is_contact` relation never establishes either field. `live_entry_type` is a current producer observation only when actually present; it is exposed separately and never overwrites the human-confirmed `entry_type`. A present mismatch appears in `classification_conflicts`, requires renewed identity/type confirmation, and keeps readiness pending. Absence of `live_entry_type`—including on an external account-relation row—preserves a confirmed `device` or `contact` kind without inventing a conflict; a new row still starts unknown and requires an answer. Entry type is not consumed as standing send permission and does not change the existing ownership confirmation rules. A recipient previously receiving a test proves that specific transfer and receipt, not permanent connectivity or future authorization. `last_confirmed_receipt` is history only. `requires_identity_confirmation: false` means the stored identity answer is complete, not that sending is allowed.
...
`annotate.py` and inventory synchronization are non-transmitting. Annotated rows expose stored `entry_type`, observed `live_entry_type` only when the producer actually supplied one, `entry_type_conflict`, `label`, `aliases`, and `notes`; none is send authorization. An external row may carry `is_contact: true` with no `live_entry_type`, preserving its human-confirmed kind without a false conflict. The synchronized structured result retains the historical `devices` name and adds current `discovery_scope`, `contacts_checked`, `unverified_contacts`, `classification_conflicts`, `unclassified_entries`, `ownership_questions`, and `initialization_status`. Nested external `recipient_devices` remain producer information and are not persisted as inventory entries.

For synchronized discovery, exit `0` means `initialization_status: "ready"`. Exit `2` means the JSON is valid but coverage is pending: ownership questions, duplicate-name ambiguity, live entries with unknown/missing stored kind, a genuine present stored/observed kind conflict, no verified current contact check, or known contacts absent from the current collection. Missing `live_entry_type` alone is not a conflict. Status priority is `awaiting_ownership_confirmation`, then `awaiting_entry_classification`, then `awaiting_contact_check`, then `ready`. Report the specific arrays and status rather than calling it a crashed query, retrying blindly, or suppressing the gap. Exit `1` means input/inventory validation failed.
