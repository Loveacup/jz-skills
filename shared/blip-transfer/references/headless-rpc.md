...
`doctor` is non-transmitting and does not need GUI or Accessibility access. It always reports the installed Blip `installed_version`/`installed_build` and whether that pair is listed in `scripts/verified-builds.json` (`build_verified`). It uses only the fixed local socket under `~/Library/Group Containers/AY8UB8KTUX.blip/Library/Caches/sock`, verifies that the socket is a socket owned by the current UID, and has no remote-address or socket override. A verified build only needs RPC reachability (`compatibility_probe: "not_needed"`). An unverified build adds warning `unverified_blip_build` and runs a read-only compatibility probe: the allowlisted state must decode without schema errors and own rows must contain exactly one self device, one own user ID, and complete names/IDs. `compatibility_probe: "passed"` makes `ok: true`; `"failed"` reports `probe_error` (schema, GetState envelope, DRPC frame or server-error codes; early EOF, timeouts and socket errors stay transport failures) and failure `compatibility_probe_failed`, meaning the adapter must be adapted before use. Missing/invalid `verified-builds.json`, a missing socket, wrong owner, or wrong filesystem type remain hard failures. `devices`, `status`, `watch` and `send` repeat the same probe for an unverified build and fail closed with `compatibility_probe_failed`; transport errors keep their own codes.

`record-verified --transfer-id UUID` is the only writer of `verified-builds.json`. It is a no-op (`already_verified: true`) for a verified build. Otherwise it requires that UUID in the private owner-only ledger `~/.agents/private/blip-transfer/unverified-build-sends.json`, which `send` writes for an unverified build after recipient selection and immediately before the create mutation, bound to the exact installed version/build. The transfer must be outgoing, `Completed` (`8`) and free of error flags; otherwise it fails with `verification_evidence_mismatch` (exit `2`) or `verification_not_completed` (the status exit code, `4` for incoming). On success it appends `{version, build, verified_on, evidence}` without any UUID, device name or path, and clears that build's ledger entries.

`devices` is read-only and works without an unlocked GUI. This adapter parses the installed (verified or probe-passing) build's allowlisted full current discovered-user collection and emits `discovery_scope: "discovered_devices_and_contacts"`. The top-level `devices` array contains own physical rows with genuinely observed `live_entry_type: "device"` and non-self external receiver rows identified technically by `is_contact: true`. `is_contact` describes the account relation used for discovery and deduplication; it is not evidence that the human-confirmed inventory `entry_type` is `contact`. External rows therefore omit `live_entry_type` and may validly retain a user-confirmed `device` or `contact` kind. Each external row has informational `recipient_devices` rows containing only `display_name`, `device_id`, `is_online`, `is_pushable`, `live`, and `is_self`; child rows are not independently enrolled into private inventory and need no name for account routing. Public external rows never include `user_id`. The scope documents the full current discovered collection, not an exhaustive address book, permission, permanent connectivity, or proof an absent person is not a Blip contact.
...
The two unary methods recovered for build `1.2.0`/`20260914015615` (unchanged in later verified builds) are:
...
For account routing, static inspection of the `1.2.0` `BlipKit` native code supplies a narrower peer-encoding contract. `PeerId.init(userID:deviceID:)` at file offset `0x1013f0` converts a nil optional device ID to an empty string; `PeerId.isPerson` at `0x101918` returns true only when the user ID is nonempty and the device ID is empty; and the native protobuf traversal at `0xa68bc` emits user field 1 while omitting empty device field 2. The same `PeerId` type is used by the create and invite events described above. This is static evidence that an account peer is represented by a user ID plus empty/omitted device ID. It is not real delivery evidence, does not establish which receiver-side device Blip chooses, and does not imply broadcast or automatic acceptance.
