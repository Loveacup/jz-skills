...
The historical `devices` array/property name is retained for compatibility even though rows may be devices or contacts. Private inventory records use optional human-confirmed `entry_type: "device"|"contact"|"unknown"`; omitted legacy values mean `unknown`. RPC `live_entry_type` is current observation only when actually present: own physical rows carry `device`, while external `is_contact` account-relation rows omit it because the relation cannot establish a human kind. Newly synchronized names always start with unknown stored kind and unconfirmed identity. Ask the human who owns each new exact name and whether it is a device, contact, or still unknown. Also ask about any current unknown or genuine present observed/stored conflict reported in `unclassified_entries` or `classification_conflicts`. Use `inventory.py confirm --entry-type ... --confirmed-by-user` only for the actual answer. Ownership, alias, name shape, `is_contact`, child devices, and producer source never supply it. Missing `live_entry_type` preserves an already confirmed kind without creating a false conflict, but it does not classify a new row.
...
For Hermes, Claude Code and OMP use a fresh session with a narrowly scoped prompt: discover `blip-transfer`, read its instructions, inspect private inventory without printing contact emails, execute `doctor`, and report raw output. Codex is unsupported because its sandbox cannot report the required lock state; do not bypass the sandbox for diagnostics or sending. This diagnostic is not a transfer session or a substitute for the per-session device/contact preflight. Do not run `prepare`, `send`, permission prompts or repeated transfers in a discovery test. Where tool allowlists are available, allow only reads and the exact doctor command.
